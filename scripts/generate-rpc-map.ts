#!/usr/bin/env bun
/**
 * Regenerates `fishnet/generated/rpc-map/` from an `rpc-build.json` export for the
 ...
 * maintenance either.
 *
 * Usage: `bun run scripts/generate-rpc-map.ts <path/to/rpc-build.json> <path/to/prefab-layouts.json>`
...
} from "../packages/capture/src/fishnet/schema/rpc-map.ts";

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const GENERATED_DIR = path.resolve(SCRIPT_DIR, "../packages/capture/src/fishnet/generated/rpc-map");
...
const USAGE = "Usage: bun run scripts/generate-rpc-map.ts <path/to/rpc-build.json> <path/to/prefab-layouts.json>";
...
function writeBehaviourFile(behaviour: DataMineBehaviour, info: BehaviourFileInfo): void {
  const rpcMapImportPath = `${depthPrefix(info.relativePath)}/schema/rpc-map.ts`;
...
function writeBroadcastsFile(broadcasts: FishNetBroadcastDefinition[]): void {
  let content = GENERATED_HEADER;
  content += `import type { FishNetBroadcastDefinition } from "../../schema/rpc-map.ts";\n\n`;
...
function writePrefabsFile(prefabs: FishNetPrefabDefinition[]): void {
  let content = GENERATED_HEADER;
  content += `import type { FishNetPrefabDefinition } from "../../schema/rpc-map.ts";\n\n`;
...
function writeDefinitionsFile(behaviours: DataMineBehaviour[], infos: BehaviourFileInfo[]): void {
  let content = GENERATED_HEADER;
  content += `import type { FishNetBehaviourDefinition } from "../../schema/rpc-map.ts";\n`;
  ...
  writeGeneratedFile("definitions", content);
  void behaviours;
}

function writeIndexFile(): void {
  let content = GENERATED_HEADER;
  content += `import type { FishNetRpcMap } from "../../schema/rpc-map.ts";\n`;
  content += `import {\n`;
  content += `  GENERATED_BEHAVIOURS,\n`;
  content += `  GENERATED_BROADCASTS,\n`;
  content += `  GENERATED_BUILD_FINGERPRINT,\n`;
  content += `  GENERATED_METADATA_VERSION,\n`;
  content += `  GENERATED_PREFAB_DEFINITIONS,\n`;
  content += `} from "./definitions.ts";\n\n`;
  content += `export const FISHNET_RPC_MAP = {\n`;
  content += `  buildFingerprint: GENERATED_BUILD_FINGERPRINT,\n`;
  content += `  metadataVersion: GENERATED_METADATA_VERSION,\n`;
  content += `  behaviours: GENERATED_BEHAVIOURS,\n`;
  content += `  broadcasts: GENERATED_BROADCASTS,\n`;
  content += `  prefabs: GENERATED_PREFAB_DEFINITIONS,\n`;
  content += `} as const satisfies FishNetRpcMap;\n`;
...
function main(): void {
  ...
  writeDefinitionsFile(wireMap.behaviours, infos);
  writeIndexFile();
