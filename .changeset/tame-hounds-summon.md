...
`SummoningComponent.SummonSkillSync` lives on the summoned object itself, not the owning actor - the fallback credits the actor named by that same component's `SummonerSync` (remembered per summon object across updates that omit it) instead of the summon's own object id.
