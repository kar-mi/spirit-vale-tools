...
function record(type: string, data: Record<string, unknown>): string {
  return JSON.stringify({
    schemaVersion: 1, sessionId: "20260716T120000000Z-a1b2c3d4", sequence: 1,
    recordedAt: "2026-07-16T12:00:00.000Z", source: "synthetic-test", type, data,
  });
}
...
describe("market capture replay", () => {
  test("accepts structured UDP records without exposing endpoint metadata", () => {
    ...
    const result = decodeMarketCaptureJsonLines(`${record("transport.packet", {
      protocol: "udp", sourceIP: "192.0.2.10", destinationIP: "198.51.100.20",
      sourcePort: 40000, destinationPort: 7007, payloadHex: "032000",
    })}\n`, tracker);
    expect(result).toEqual({ datagrams: 1, marketEvents: 0, invalidLines: 0, decodeWarnings: 0 });
    expect(tracker.snapshot().listings).toEqual([]);
  });
