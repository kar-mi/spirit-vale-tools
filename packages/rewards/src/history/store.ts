...
export class RewardHistoryStore {
  ...
  listKills(query: ListRewardKillsQuery): Page<RecordedMobRewardKill> {
    ...
    return { items: page.map((row) => ({ kind: "kill" as const, id: row.kill_id, tick: num(row.tick), mob: { objectId: num(row.object_id), mobId: row.mob_id, displayName: row.display_name, level: num(row.mob_level), ...(row.mob_rank === null ? {} : { rank: num(row.mob_rank) }), boss: num(row.boss) === 1 }, experience: num(row.experience), jobExperience: num(row.job_experience), coins: row.coins, drops: this.drops(query.sessionId, row.kill_id), recordedAt: new Date(num(row.recorded_at_ms)).toISOString() })), ...(rows.length > limit && last ? { nextCursor: encode({ sequence: num(last.sequence), killId: last.kill_id }) } : {}) };
  }
  ...
  private listMobs(sessionId: string): MobRewardMobSummary[] { return (this.model.bigintStatement(REWARD_SQL.listMobs).all({ sessionId }) as MobRow[]).map((row) => { const drops = this.model.statement(REWARD_SQL.listMobDrops).all({ sessionId, mobId: row.mob_id }) as DropRow[]; return { mobId: row.mob_id, displayName: row.display_name, level: num(row.mob_level), boss: num(row.boss) === 1, kills: num(row.kills), experience: num(row.experience), jobExperience: num(row.job_experience), coins: row.coins, drops: drops.map(dropItem) }; }); }
...
interface KillRow { kill_id: string; sequence: number; recorded_at_ms: number; tick: number; mob_id: string; display_name: string; mob_level: number; mob_rank: number | null; boss: number; object_id: number; experience: number; job_experience: number; coins: bigint }
interface MobRow { mob_id: string; display_name: string; mob_level: number; boss: number; kills: number; experience: number; job_experience: number; coins: bigint }
