...
export const REWARDS_DOMAIN_VERSION = 2;

const SCHEMA = `
create table if not exists reward_kills (session_id text not null, kill_id text not null, sequence integer not null, recorded_at_ms integer not null, tick integer not null, mob_id text not null, display_name text not null, mob_level integer not null, mob_rank integer, boss integer not null, object_id integer not null, experience integer not null, job_experience integer not null, coins integer not null, primary key(session_id, kill_id));
...
create table if not exists reward_mob_totals (session_id text not null, mob_id text not null, display_name text not null, mob_level integer not null, boss integer not null, kills integer not null, experience integer not null, job_experience integer not null, coins integer not null, primary key(session_id, mob_id));
