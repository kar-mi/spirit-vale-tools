...
- c7e1380: Suppress the redundant packet copies a redirecting VPN puts on the wire.
...
- cf80075: A packet waiting to be attributed to the target process is now held for several endpoint-table refreshes rather than one.
  
  Neither endpoint of a freshly opened socket has been reported yet, so every connection's first packets are held. One refresh gave them a single chance to land, and often it did not: the opening of a connection was discarded on that race, the connect and the authentication with it. Packets given up on are now reported as a warning instead of dropped silently.
