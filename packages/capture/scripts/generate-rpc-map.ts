...
function loadWireMap(inputFile: string): DataMineWireMap {
  if (!existsSync(inputFile)) {
    throw new Error(`rpc-build.json not found at ${inputFile}. Pass its path as an argument, or refresh the data-mine (pipelines.refresh_all) first.`);
  }
 ...
 * missing, since older data-mine checkouts may not have it yet - `prefabs.ts` just won't
 ...
 * than generated flat. Verify that split still covers exactly the RPCs the data-mine
